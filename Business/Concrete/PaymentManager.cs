    ...
    public class PaymentManager : IPaymentService
    {
        ...
        public IDataResult<int> Pay(CreditCard creditCard, int customerId, decimal amount)
        {
            ...
            if (result.Success)
            {
                ...
                var paymentId = _paymentDal.Get(p => p.CustomerId == customerId
                                                && p.Amount == amount
                                                && p.CreditCardId == creditCard.Id
                                                && (p.PaymentDate.Date == paymentDate.Date
                                                && p.PaymentDate.Hour == paymentDate.Hour
                                                && p.PaymentDate.Second == paymentDate.Second)).Id;
